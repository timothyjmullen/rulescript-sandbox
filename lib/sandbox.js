...
/* DOCUMENTS
 **********************************************************/
function createDocument(name, data, callback) {
  var session = new EditSession('');
  session.setUndoManager(new UndoManager());
  var doc = {'name': name, 'session': session, 'dat': data || {'msgvar':{}}, 'uuidx': ''};
...
// Add new tab
function addTab(first, name, data) {
    if (first == true) {
      var tabs = tabUL;
      ...
      var nt = document.createElement('li');
      if (fileCache[name]) {name = name + ' (1)'}
      if (fileCache[name]) {name = name + ' (2)'}
