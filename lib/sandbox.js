...
function closeTab() {
  ...
  if (/active/.test(li.getAttribute('class')) && ul.childNodes.length > 1) {
    if (ul.childNodes[index].nextSibling) {
      ul.childNodes[index].nextSibling.setAttribute('class', 'active');
    } else {
      ul.childNodes[index].lastChild.setAttribute('class', 'active');
    }
...
function activeTab(n) {
  var tabs = container.querySelectorAll(".active");
  for (var k = 0; k < tabs.length; k++) {
    tabs[k].removeAttribute('class');
  }
  if (n == 'n') {
    document.getElementById('tablist').lastChild.setAttribute('class', 'active');
  } else {
