 ...
 **********************************************************/
 function dataPop(warn) {
    ...
    if (warn) {
      pcon.innerHTML = pcon.innerHTML + '<br style="clear:both;"" /><span class="warn">* Attribute name already exists! *</span>';
    }
    popDataEvents();
...
document.getElementById("tsubmit").addEventListener("click", function(){
  if (typeof env.dat[document.getElementById('pop_in_tab').value] === "undefined"){
    addData(document.getElementById('pop_in_tab').value);
    closePop();
  } else {
    dataPop(true);
  }
