...
/* DOCUMENTS
 **********************************************************/
function createDocument(name, data, callback, collab, help) {
  var session = new EditSession('');
  session.setUndoManager(new UndoManager());
  var doc = {'name': name, 'session': session, 'dat': data || {'msgvar':{}}, 'uuidx': '', collab: collab || '', help: help || false};
  doc.dat.msgvar = {};
...
function swapDocument(name) {
  ...
  // Help documentation
  if (fileCache[name].help) {
  	env.editor.setValue(atob(helpCopy));
	env.editor.session.selection.moveCursorFileStart();
...
// Add new tab
//-------------------------------
function addTab(first, name, data, collab, help) {
    if (first == true) {
      var tabs = tabUL;
...
document.getElementById("help").addEventListener("click", function () {
    addTab(false, "Help Reference",false,false,true);
});
